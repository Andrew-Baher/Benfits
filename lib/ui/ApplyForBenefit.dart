...
String globalTitle = '';
int BenefitQuestions;
String formattedDate;
bool _saving = false;
int incNoOfAnswers = 0;
String noOfAnswers = '';

class ApplyForBenefit extends StatefulWidget {
  @override
  _ApplyForBenefitState createState() => new _ApplyForBenefitState();
}

class _ApplyForBenefitState extends State<ApplyForBenefit>
              ...
              children: <Widget>[
                Row(
                  children: <Widget>[
                    ...
                    Text(
                      makeBenefitQuestions[index].questionTitle,
                      style: TextStyle(
                          color: Colors.black,
                ...
                child: Column(
                  children: <Widget>[
                    ListView.builder(
                      ...
                      itemCount: makeBenefitQuestions.length,
                        ...
                        return Container(
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              (!makeBenefitQuestions[index].questionType)
                                  ? TextField(
                                  autofocus: false,
                                  onChanged: (text) {
                                    Control[index] = text;
                                  ...
                                  : Column(
                                children: makeBenefitQuestions[index]
                                    .questionChoice
                                    .map((t) => RadioListTile(
                                  title: Text("$t"),
                                  value: t,
  ...
  void getBenefit() async {
    ...
    DBRef2.child('benefit'+mainCurrentBenefitID).once().then((DataSnapshot dataSnapShot) {
      ...
      globalTitle = dataSnapShot.value["benefitTitle"]["Title"];
      BenefitQuestions = dataSnapShot.value["QuestionsCount"]["count"];
      int count = 0;
      Question question;
      for (int i = 0; i < BenefitQuestions; ++i) {
        ...
        makeBenefitQuestions.add(question);
        count++;
      }
      print(count);
    });
  ...
  Future sendAnswers(String count, int i) async {
    DBRef4.child('BenefitAnswer'+mainCurrentBenefitID).child(noOfAnswers).child(count).set({
      "Answer": Control[i],
    });
  }
  ...
  void _saveSurveyAnswer() async {
    ...
    DBRef4.child('BenefitAnswer'+mainCurrentBenefitID).child(noOfAnswers).set({
    ...
    for (int i = 0; i < makeBenefitQuestions.length; i++) {
      await new Future.delayed(const Duration(seconds: 1));
      await sendAnswers(i.toString(), i);
    }

    DBRef.child('benefit'+mainCurrentBenefitID).child("AnswersCount").set({
